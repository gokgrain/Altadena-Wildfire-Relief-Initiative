...
const STATUS_COLOR = { todo: '#f43f5e', 'in-progress': '#f59e0b', done: '#10b981' }
const HABIT_PALETTE = ['#7c5cfc', '#06b6d4', '#10b981', '#f59e0b', '#f43f5e', '#6366f1']

// ── Chart Components ──────────────────────────────────────
function LineChart({ id, values, color = '#7c5cfc', height = 64 }) {
  const nonNull = values.filter(v => v !== null)
  if (!nonNull.length) return (
    <div style={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <span style={{ fontSize: 10, color: '#ffffff20' }}>데이터 없음</span>
    </div>
  )
...
function DonutChart({ done, total, color = '#a78bfa', size = 76 }) {
  const rate = total > 0 ? done / total : 0
  const r = 26, circ = 2 * Math.PI * r
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg viewBox="0 0 64 64" width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        <circle cx="32" cy="32" r={r} fill="none" stroke="#ffffff08" strokeWidth="8" />
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontSize: 15, fontWeight: 700, color: '#ffffff70', lineHeight: 1 }}>{Math.round(rate * 100)}%</span>
        <span style={{ fontSize: 8, color: '#ffffff30', marginTop: 1 }}>{done}/{total}</span>
      </div>
    </div>
  )
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      {data.map(({ label, rate, total }, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 14, fontSize: 9, color: '#ffffff30', textAlign: 'right', flexShrink: 0 }}>{label}</span>
          <div style={{ flex: 1, height: 5, background: '#ffffff08', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{
              height: '100%', borderRadius: 3,
              width: `${(rate / maxRate) * 100}%`,
              background: rate > 70 ? '#10b981' : rate > 40 ? '#f59e0b' : total === 0 ? '#ffffff10' : '#f43f5e',
              transition: 'width 0.4s',
            }} />
          </div>
          <span style={{ width: 28, fontSize: 9, color: '#ffffff30', textAlign: 'right', flexShrink: 0 }}>
    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: BAR_H + 28 }}>
      {data.map(({ label, value, color }, i) => (
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          <span style={{ fontSize: 9, color: '#ffffff40' }}>{value}</span>
          ...
          <span style={{ fontSize: 8, color: '#ffffff25', textAlign: 'center', width: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {label}
          </span>
        </div>
      ))}
    ...
    <div style={{ display: 'flex', gap: 4, marginTop: 10 }}>
      {Object.entries(dist).map(([k, v]) => (
        <div key={k} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          <div style={{ width: '100%', height: 4, background: v > 0 ? color : '#ffffff0a', borderRadius: 2, opacity: 0.4 + (v / max) * 0.6 }} />
          <span style={{ fontSize: 7, color: '#ffffff20', textAlign: 'center' }}>{k}</span>
          <span style={{ fontSize: 9, color: '#ffffff35', textAlign: 'center' }}>{v}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="panel rounded-lg" style={{ padding: '10px 12px', flexShrink: 0 }}>
      <div style={{ marginBottom: 8 }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: '#ffffff45', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{title}</div>
        {subtitle && <div style={{ fontSize: 9, color: '#ffffff20', marginTop: 1 }}>{subtitle}</div>}
      </div>
      {children}
    </div>
  )
...
function MonthlyCalendar({ year, month, timeboxBlocks }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', flexShrink: 0, borderBottom: '1px solid #ffffff08' }}>
        {DAYS.map(d => (
          <div key={d} style={{ textAlign: 'center', padding: '5px 0', fontSize: 9, color: '#ffffff20' }}>{d}</div>
        ))}
      </div>
        ...
        {cells.map((cell, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              borderRight: col < 6 ? '1px solid #ffffff04' : 'none',
              borderBottom: row < 5 ? '1px solid #ffffff04' : 'none',
              background: isToday ? '#7c5cfc09' : 'transparent',
            }}>
              <div style={{
                fontSize: 9, width: 16, height: 16, flexShrink: 0, marginBottom: 2,
                display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%',
                fontWeight: isToday ? 700 : 400,
                background: isToday ? '#7c5cfc35' : 'transparent',
                color: isToday ? '#a78bfa' : cell.cur ? '#ffffff35' : '#ffffff12',
              }}>
                {cell.date.getDate()}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 1.5, overflow: 'hidden', flex: 1 }}>
                {blocks.slice(0, 4).map(block => (
                  <div key={block.id} style={{ display: 'flex', alignItems: 'center', gap: 3, minWidth: 0 }}>
                    <div style={{ width: 4, height: 4, borderRadius: 2, flexShrink: 0, background: STATUS_COLOR[block.status] || '#ffffff30' }} />
                    <span style={{ fontSize: 8, color: '#ffffff40', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, lineHeight: 1.3 }}>
                      {block.text}
                    </span>
                  </div>
                ))}
                {blocks.length > 4 && <span style={{ fontSize: 8, color: '#ffffff20' }}>+{blocks.length - 4}</span>}
              </div>
            </div>
          )
        })}
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '2px 4px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: '#ffffff60', minWidth: 80, textAlign: 'center' }}>
            {year}년 {month + 1}월
          </span>
          <button className="icon-btn" onClick={nextMonth}><ChevronRight size={13} /></button>
        </div>
        <span style={{ fontSize: 9, color: '#ffffff20', letterSpacing: '0.1em' }}>SCHEDULER ANALYTICS</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', gap: 8, overflow: 'hidden' }}>
        ...
        <div className="panel rounded-lg" style={{ flex: '0 0 44%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <div className="panel-header">
            ...
            <span style={{ fontSize: 9, color: '#ffffff20' }}>● 완료 ● 진행중 ● 미완료</span>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <StatCard title="주간 계획 실행률" subtitle="최근 8주 · 완료 블록 / 전체 블록">
            <LineChart id="exec" values={weeklyExecution.map(w => w.rate)} color="#7c5cfc" height={64} />
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 3, paddingInline: 2 }}>
              {weeklyExecution.map((w, i) => (
                <span key={i} style={{ fontSize: 7, color: i === 7 ? '#a78bfa' : '#ffffff18' }}>{w.label}</span>
              ))}
            </div>
          </StatCard>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            <StatCard title="Must Todo 달성률" subtitle={`${year}년 ${month + 1}월`}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <DonutChart done={mustStats.done} total={mustStats.total} color="#a78bfa" />
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: '#ffffff60', lineHeight: 1 }}>{mustStats.done}</div>
                    <div style={{ fontSize: 9, color: '#ffffff30' }}>완료</div>
                  </div>
                  <div style={{ width: 1, height: 16, background: '#ffffff10' }} />
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: '#ffffff30', lineHeight: 1 }}>{mustStats.active}</div>
                    <div style={{ fontSize: 9, color: '#ffffff20' }}>진행중</div>
                  </div>
                </div>
              </div>
            </StatCard>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            <StatCard title="평균 블록 소요 시간" subtitle={`완료 ${durationStats.count}개 기준`}>
              <div style={{ fontSize: 26, fontWeight: 700, color: '#ffffff55', lineHeight: 1 }}>
                {durationStats.label}
              </div>
              <MiniDistBar dist={durationStats.dist} color="#f59e0b" />
            </StatCard>
            <StatCard title="Brain Dump → 완료" subtitle={`${sojournStats.count}건 · 평균 소요 일수`}>
              <div style={{ fontSize: 26, fontWeight: 700, color: '#ffffff55', lineHeight: 1 }}>
                {sojournStats.avg !== null ? `${sojournStats.avg}일` : '–'}
              </div>
              <MiniDistBar dist={sojournStats.dist} color="#06b6d4" />
            </StatCard>
          </div>
          ...
          <StatCard title="습관 달성률 추이" subtitle="최근 12주 주간 평균">
            {habitHistory.length > 0 ? (
              <>
                <LineChart id="habit" values={habitHistory} color="#10b981" height={64} />
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 2, paddingInline: 2 }}>
                  {habitHistory.map((_, i) => (
                    <span key={i} style={{ fontSize: 7, color: i === 11 ? '#10b981' : '#ffffff15' }}>
              ...
              <div style={{ height: 64, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 10, color: '#ffffff20' }}>습관 데이터 없음</span>
              </div>
            )}
          </StatCard>
